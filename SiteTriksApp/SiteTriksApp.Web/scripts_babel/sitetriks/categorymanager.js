...
function createCategory(storeId) {
    ...
    function createCategoryForm(ev) {
        ...
        if (!Validator.validate($title, 'Title must be atleast 3 characters long!', function (val) {
        ...
        Data.postForm({ url: '/ecommerse/categorymanager/validatecategory', formData: validationForm }).then(function (res) {
            if (res.success) {
                ...
                return Data.postForm({ url: $form[0].action, formData: formData });
            }

            Validator.validate($title, res.message || 'Name is already taken!', function (val) {
                return res.success;
            });
            Loader.hide();
