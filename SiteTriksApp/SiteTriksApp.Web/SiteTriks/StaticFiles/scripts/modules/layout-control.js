...
export function initLayout($wrapper, l, $resolutions, $options, resolutionValidation) {
    ...
    const templateSources = templateNames.map((templateName) => { return { name: templateName, url: `/SiteTriks/StaticFiles/templates/layout/${templateName}.html` }; });
    const _resolutions = ['xs', 'sm', 'md', 'lg'];

    l.resolutions = ['xs', 'sm', 'md', 'lg'];
    l.deletedPlaceholders = [];

    loadHandlebarsTemplates(templates, templateSources).then(function (res) {
